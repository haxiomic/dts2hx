...
class Main {
	...
	static function convertTsDefinitions(moduleId: String, entryPointFilePath: String, compilerOptions: CompilerOptions, outputDirectory: String, outputFlags: EnumFlags<OutputType>) {
		...
		function writeModules(modules: Map<String, HaxeModule>, outputType: OutputType) {
			...
			// write a readme
			var readmeStr = generateReadme(converter, outputType);
			var readmePath = Path.join([modulePath, 'README.md']);
			touchDirectoryPath(Path.directory(readmePath));
	...
	static function generateReadme(converter: ConverterContext, outputType: OutputType): String {
		var repoUrl = packageJson.repository.url;
		var dts2hxRef = repoUrl != null ? '[dts2hx]($repoUrl)' : 'dts2hx';
		return '
			# Haxe Externs for ${converter.entryPointModuleId}
			...
			${switch outputType {
				...
				case Modular: 'These definitions use JavaScript modules, `require("${converter.entryPointModuleId.removeQuotes()}")` is used to load the library at runtime. When targeting the web, you may want to use a JavaScript bundler to combine modules into a single file.';
			}}
