...
typedef HRTime = {
	...
	**/
	...
	/**
		...
		method returning the current high-resolution real time in nanoseconds as a `bigint`.
		
		Unlike
		{@link
		process.hrtime()
		}
		, it does not support an additional time argument since the difference can just be computed directly by subtraction of the two `bigint`s.
		```js
